...
with col2:
    ...
    gb.configure_column("Hora", cellStyle={'color': 'green'})
...
st.markdown("<br>" * 2, unsafe_allow_html=True)
st.markdown(
    '<h3 id="evolucion-anual" class="titulo-secundario" style="margin-top: 30px; margin-bottom: 60px;"> Clasificación y Pronóstico de Inversion </h3>',
    unsafe_allow_html=True
)
...
with col1:
    st.markdown(
        f'<div class="menu-lateral-grande" style="font-size: 20px> Moneda seleccionada: <span>{filtro_moneda}</span></div>',
        unsafe_allow_html=True
    )
...
st.subheader("Clasificación de Inversión")
st.markdown("---")
st.markdown("<br>", unsafe_allow_html=True)
col_rangos, col_kmeans = st.columns(2)
#NOTE: ---------------------------------------------------------------- RANGOS FIJOS
with col_rangos:
    st.markdown("##### Clasificación por Rangos Fijos ")
    st.markdown("###### Monto Invertido en Dolares ")
    df_moneda["Grupo_Fijo"] = pd.cut(df_moneda["Inversion"], bins=[-float("inf"), 10000, 50000, float("inf")], labels=["Baja <10, 000", "Media 10,000 - 50,000", "Alta >100,000"])
    ...
    fig_fijo = px.bar(
        ...
        color_discrete_map={"Baja <10, 000": "#a6cee3", "Media 10,000 - 50,000": "#1f78b4", "Alta >100,000": "#08306b"},
        template="plotly_white"
    )
...
with col_kmeans:
    st.markdown("##### Agrupamiento con KMeans")
    st.markdown("###### De acuerdo a clusters de invesión")
    try:
        ...
        fig2 = px.scatter(
            ...
            labels={"Fecha": "Fecha de transacción", "Inversion": "Monto Invertido (Dolares)"},
            template="plotly_white"
        )
...
#* Asignamos colores base por criptomoneda
colores_base = {
    'Bitcoin': 'rgb(255, 99, 132)',
    'Ethereum': 'rgb(54, 162, 235)',
    'Binance Coin': 'rgb(255, 206, 86)',
    'Solana': 'rgb(75, 192, 192)',
    'Bitcoin Cash': 'rgb(153, 102, 255)'
}


#* Obtenemos el tono trasnparente del color de cada criptomenada
def rgb_a_rgba(rgb_str, alpha=0.05):
...
st.markdown("<br>" * 3, unsafe_allow_html=True)
st.markdown(
    '<h4 class="titulo-secundario" style="margin-top: 60px;">Evolución Anual del Volumen de Transacciones por Criptomoneda</h3>', unsafe_allow_html=True
)
...
fig_line_vol_anual = px.line(
    ...
    color="Moneda",
    markers=True,
    title="Volumen Anual de Transacciones",
    labels={"Volumen": "Total Transacciones", "Año": "Año"},
    template="plotly_white",
    color_discrete_map=colores_base #*Mapeamos los colores para cada criptomoneda
)
...
#* Aplicamos la transparencia al color de línea original
for trace in fig_line_vol_anual.data:
    color_rgb = colores_base.get(trace.name, 'rgb(0,0,0)')
    trace.update(
        fill='tozeroy',
        fillcolor=rgb_a_rgba(color_rgb, 0.09),
